...
class MealyMachine:
    ...
    def process_input(self, input_sequence):
        ...
        tout en enregistrant l'évolution des états.
        :param input_sequence: Liste des entrées.
        :return: Tuple (liste des sorties, liste des états visités).
        """
        outputs = []
        states = [self.current_state]  # Initialiser avec l'état courant
        for input_symbol in input_sequence:
            if (self.current_state, input_symbol) in self.transitions:
                ...
                states.append(next_state)  # Ajouter le nouvel état
            else:
...
def execute_tests(mealy_machine, test_sequences):
    ...
    y compris l'évolution des états.
    :param mealy_machine: Instance de MealyMachine.
    ...
    for sequence in test_sequences:
        ...
        try:
            outputs, states = mealy_machine.process_input(sequence)  # Traiter la séquence d'entrée
...
for test, output, states in execute_tests(mealy_machine, generate_simple_tests(mealy_machine)):
    print(f"Entrée : {test} -> Sortie : {output}, États visités : {states}")

print("\nTests Complexes (Méthode Complexe) :")
for test, output, states in execute_tests(mealy_machine, generate_complex_tests(mealy_machine, max_length=3)):
    print(f"Entrée : {test} -> Sortie : {output}, États visités : {states}")
